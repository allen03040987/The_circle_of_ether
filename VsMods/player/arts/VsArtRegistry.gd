...
## 武藝名稱字串 → 腳本路徑的對照表。VsGameManager.CLOTTY_ARTS 只存純字串
## （不像主遊戲 equipped_martial_arts 直接存完整資源路徑），所以需要這一層
## 查表——VsPlayer._load_arts() 施放時用 art_slots 裡的字串來這裡查腳本。
##
## 目前只有第一個武藝有實際腳本（見 project_vsmods_rebuild.md 的「武藝範例」
## 任務），其餘 5 個名稱先留在 VsGameManager.CLOTTY_ARTS 裡但查不到腳本——
## get_art_script() 找不到就回傳 null，VsPlayer._load_arts() 對應處理成
## 「這個槽位選了但沒東西」，不會報錯，之後陸續補上就好。
