...
**⚠ Redeploying this service can fail with `Timed Out` even though the process starts fine** — confirmed via deploy log showing `Signaling server 啟動，監聽 :PORT` printed successfully, then `Timed Out`. Root cause: Render's health check is a plain HTTP GET, but this server only speaks the WebSocket handshake — without a `process_request` handler, an HTTP request gets rejected by the `websockets` library before ever reaching application code, so the health check never gets a response and Render kills the deploy. Fixed with a `process_request` callback passed to `websockets.serve(..., process_request=_health_check)` that returns `connection.respond(200, "OK\n")` for any plain HTTP request. This appears to be the **first redeploy since the service's initial setup** — the original deploy apparently never exercised this path, so it's easy to assume redeploys "just work" until you actually trigger one.
