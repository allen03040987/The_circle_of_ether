...
- **空中普攻** — `VsAirAttack.gd`，獨立狀態（不是在 VsAttack 裡加分支），3 段連段，動畫時間軸/hitbox 數值/spawn_anim_vfx 呼叫從主遊戲 `Katana.gd` 的 `DICT_LIGHT_AIR`＋`player.tscn` 的 `air_light_1~3` 動畫逐一移植（素材 `player/Katana/air_A.png`，是一張 5×4 的 spritesheet，三段共用同一張圖、切不同 frame 區段，不是三張獨立圖）。判定框 `HitboxAir1~3`（跟地面 `HitboxA1~5` 是同一個 `VsPlayer.hitboxes` 集合，`_ready()` 通用收集）。從 `VsJump`/`VsFall` 的 `input.attack` 進入（且 `not VsPlayer.air_attack_used`，見下）；落地當幀立刻收招（空中連段不能延續到地面，`_grounded()` 一旦為真直接走 `_recovery_transition`）；重力**完全比照主遊戲**：第 1/2 段一般重力，只有第 3 段（終結技，對應主遊戲 `combo_step==13`）套用「近零重力滯空」（`velocity.y += GRAVITY * 0.1 * delta`，原樣照搬 `new_y += default_gravity * 0.1 * delta`）。⚠ 曾經一度改成 3 段全部套用近零重力（因為使用者反映 VsMods 跳躍初速較低、1/2 段直接墜地），但使用者後來要求改回「完全按照主世界的效果」，所以現在是精確 1:1 port，不要再擅自加大滯空範圍。⚠ 主遊戲同一招表面上看似還有「延遲上勾噴射」，但那其實是另一招（`combo_step==23`，地面戰技昇龍斬）的效果，跟 `air_light_1~3` 無關，不是漏移植。判定框大小/位置是我抓的合理預設值（主遊戲三段共用同一顆 hitbox 沒有分開的數值可抄），建議進編輯器微調。**每次跳躍限用一輪**：`VsPlayer.air_attack_used`（落地時歸零，比照主遊戲 `air_attack_locked`），`VsAirAttack.enter()` 開始新一輪時設 true，`VsJump`/`VsFall` 的 `input.attack` 入口檢查此旗標——目前沒有移植主遊戲「衝刺可刷新這次機會」的 `refresh_air_skill_on_dash()`。**上升推力**：主遊戲 `_play_air_step()` 在**每一段**觸發當下都直接 `player.velocity.y = air_thrust_force`（-150.0，往上）——不是只有第一段，連段每一下都會把角色往上頂一點。`VsAirAttack.AIR_THRUST_FORCE = -150.0` 在 `enter()` 原樣照搬（一開始漏做，2026-07-18 使用者指出後補上）。
